...
- `response_format=url` 不支持，接口固定返回 `b64_json`；默认请求传入 `url` 会直接返回错误。开启 `override-response-format-url` 后会自动按 `b64_json` 处理。
...
- `override-response-format-url` 和 `override-transparent-background` 默认关闭，分别控制对应参数覆盖；其它不支持参数仍会返回错误。
